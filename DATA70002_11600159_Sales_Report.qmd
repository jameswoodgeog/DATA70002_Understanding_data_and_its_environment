...
```{r}
# Get the average sales per week of year
hist_avg_sales_per_week <- df |> group_by(Year, WeekOfYear) |> summarise(Avg_Sales = mean(Sales, na.rm = TRUE))
str(hist_avg_sales_per_week)

# Get the predicted average sales per week
pred_avg_sales_per_week <- sales_prediction_data |> group_by(Year, WeekOfYear) |> summarise(Avg_Sales = mean(Sales, na.rm = TRUE))
str(sales_prediction_data)

# Plot them both on the y axis and the week of year on the x axis using ggplot2
ggplot(hist_avg_sales_per_week, aes(x = WeekOfYear, y = Avg_Sales)) +
  geom_line(aes(y = Avg_Sales, color = "Actual Sales")) +
  geom_line(data = pred_avg_sales_per_week, aes(y = Avg_Sales, color = "Predicted Sales")) +
...
# and plot using ggplot2 this in the y axs against the week of year on the x axis
ggplot(hist_avg_sales_per_week, aes(x = WeekOfYear, y = Avg_Sales)) +
  geom_line() +
  labs(title = "Average Sales per Week of Year",
       x = "Week of Year",
       y = "Average Sales") +
  theme_minimal()
```
...
hist_sales <- df |> filter(df$Store %in% sales_prediction_data$Store)
...
# Get the predicted average sales per week
# hist_sales <- hist_sales |> group_by(Year, WeekOfYear) |> summarise(Avg_Sales = mean(Sales, na.rm = TRUE))
hist_sales <- hist_sales |> group_by(Store, Year, Month, WeekOfYear, Date) |> summarise(avg_hist_sales = mean(Sales, na.rm = TRUE))
str(hist_sales)

hist_sales$WeekOfYear <- as.numeric(hist_sales$WeekOfYear)
str(hist_sales)
```


```{r}
# plot the hist_sales on the y axis and predicted sales on the y axis and the relevant months on the x axis
# Ensure WeekOfYear is treated as a numeric variable
hist_sales$WeekOfYear <- as.numeric(hist_sales$WeekOfYear)

# Order the data by WeekOfYear
hist_sales <- hist_sales[order(hist_sales$WeekOfYear), ]

# Plot the data with geom_smooth
ggplot(hist_sales, aes(x = WeekOfYear, y = avg_hist_sales)) +
  geom_smooth(aes(color = "Predicted Sales"), method = "loess", se = FALSE) +
  labs(title = "Actual vs Predicted Sales per Week of Year",
       x = "Week of Year",
       y = "Sales",
       color = "Sales Type") +
  scale_color_manual(values = c("Predicted Sales" = "red")) +
  theme_minimal()
```
