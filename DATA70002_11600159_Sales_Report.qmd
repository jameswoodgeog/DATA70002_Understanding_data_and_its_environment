...
remove(tune_grid, dt_model)

# Do RMSPE
# Remove missing values from actual and predicted values
actual_values <- validation_set$Sales[!is.na(validation_set$Sales)]
predicted_values <- tree_predictions[!is.na(validation_set$Sales)]
...
RMSPE_value <- RMSPE(actual_values, predicted_values)

# Print RMSPE
cat("RMSPE:", round(RMSPE_value*100, 2),"%", "\n")


# remove(actual_values, mean_squared_percentage_error, percentage_error, predicted_values, RMSPE, RMSPE_formatted, RMSPE_rounded, RMSPE_value, squared_percentage_error, tree_predictions, tune_grid)
```
...
# Remove missing values from actual and predicted values
actual_values <- validation_set$Sales[!is.na(validation_set$Sales)]
predicted_values <- rf_predictions[!is.na(rf_predictions)]
...
# Remove missing values from actual and predicted values
actual_values <- validation_set$Sales[!is.na(validation_set$Sales)]
predicted_values <- xgb_predictions[!is.na(xgb_predictions)]
